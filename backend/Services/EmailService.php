...
class EmailService
{
    ...
    public function genereteRecupCode($email_user)
    {
        ...
        $bodyEmail = [
            ...
            '{{mensagem}}'    => 'Recebemos seu pedido de redefinição de senha , este codigo vale por 5 minutos.',
            '{{texto_botao}}' => "{$codigoRecup}",
            '{{observacao}}'  => 'Se você não solicitou a redefinição de senha, ignore este e-mail.',
        ];
    ...
    public function genereteReativCode($email_user)
    {
        ...
        $bodyEmail = [
            '{{titulo}}'      => 'Solicitação de reativação de conta',
            '{{nome}}'        => "{$usuario->primeiro_nome} {$usuario->ultimo_nome}",
